...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    int size=0,line=0;
    struct obj_t obj;
    if(fscanf(file,"count=%d",&size)!=1 || size<=0)
    {
         fprintf(stderr,"Nespravny format poctu objektov!!\n");
         return -1;
    }

    *arr=malloc(sizeof(struct cluster_t) * size);
    if(*arr==NULL)
    {
    ...
    while(line<=size-1)
    {
        if(line==size-1)
        {
            if(fscanf(file,"%d %f %f", &obj.id, &obj.x, &obj.y)==3)
            {
                init_cluster(&(*arr)[line],1);
                append_cluster(&(*arr)[line],obj);
            }
            else
            {
                fprintf(stderr,"Nepodarilo sa nacitat objekty!!\n");
                return -1;
            }
        }
        else
        {
            if(fscanf(file,"%d %f %f%c", &obj.id, &obj.x, &obj.y, &test)==4 && test=='\n')
            {
                init_cluster(&(*arr)[line],1);
                append_cluster(&(*arr)[line],obj);
            }
    ...
    if(fclose(file))
    {
        fprintf(stderr,"Nepodarilo sa zatvorit priecinok!!\n");
        return -1;
    }
    return size;
}
...
int main(int argc, char *argv[])
{
    ...
    if(cluster_num>size)
    {
        fprintf(stderr,"Nemozny pocet objektov!!\n");
        return 1;
    }

    while(size>cluster_num)
    {

...
**/
	tBTNodePtr pointer = (*RootPtr);
	bool foundBT = FALSE;
	tBTNodePtr positionBT;
	if ((*RootPtr) != NULL)	//bvs nie je prazdny
	{
		do
		{	
			positionBT = pointer;
			if (pointer->Cont > Content)	//vkladany obsah je mensi idem do lava
			{
				pointer = pointer->LPtr;
			}
			else if (pointer->Cont < Content)	//obsah je vacsi idem do prava
			{
				pointer = pointer->RPtr;
			}
			else	//rovnaky obsah
				foundBT = TRUE;
		}while((pointer != NULL) && (foundBT == FALSE));
	}
	else	//prazdny BVS
	{
		positionBT = NULL;
	...
	if (foundBT == FALSE)
	{
		tBTNodePtr temp = malloc(sizeof(struct tBTNode));
		temp->Cont = Content;
		temp->LPtr = NULL;
		temp->RPtr = NULL;
		if (positionBT != NULL)	//iba v pripade ze strom nie je prazdny
			{
				if (positionBT->Cont > Content)
				{
					positionBT->LPtr = temp;
				}
				else
				{
					positionBT->RPtr = temp;
				}
			}	
		else	//strom bude prazdny tak pouzijeme novy uzol
			(*RootPtr) = temp;
	}
	else
		positionBT->Cont = Content;
}
...
**/
	while(ptr != NULL)
	{
		SPushP(Stack, ptr);	//push na zasobnik
		BTWorkOut(ptr);	//spracovanie uzla
		ptr = ptr->LPtr;	//postup do lava
	}
}
...
**/
	tStackP stack;
	SInitP(&stack); //inicializacia zasobniku ukazatelov
	Leftmost_Preorder(RootPtr, &stack); //uzly najviac na lavo
	tBTNodePtr pointer = RootPtr;	
	while(!SEmptyP(&stack))	//cyklus do konca zasobniku
	{
		pointer = STopPopP(&stack);	//uzol z vrcholu a pop
		Leftmost_Preorder(pointer->RPtr, &stack);	//uzol najviac na lavo
	}	
}
...
** zásobníku. 
**/
	while(ptr != NULL)	//prejdenie uzlov
	{
		SPushP(Stack, ptr);	//ukladanie ukazatelov na zasobnik
		ptr = ptr->LPtr;	//posun do dalsieho uzla po lavej strane
	}
}
...
**/
	tStackP stack;
	SInitP(&stack);
	tBTNodePtr pointer = RootPtr;	//ukazatel 
	Leftmost_Inorder(RootPtr, &stack);	//najlavsie uzly
	while(!SEmptyP(&stack))
	{
		pointer = STopPopP(&stack);	//nacitanie uzla
		BTWorkOut(pointer);	//spracovanie
		Leftmost_Inorder(pointer->RPtr, &stack);	//najlavsi uzol -> pravemu podstromu
	}	
}
...
**/
	while(ptr != NULL)
	{
		SPushP(StackP, ptr);	//ukazatel na uzol v zasobniku
		SPushB(StackB, true);
		ptr = ptr->LPtr;	//postup do lava
	}
}
...
**/
	if (RootPtr != NULL)	//je s cim pracovat
	{
		bool left;
		tStackP stack;	//zasobniky
		SInitP(&stack);
		tStackB stackBool;
		SInitB(&stackBool);
		tBTNodePtr pointer = RootPtr;
		Leftmost_Postorder(pointer, &stack, &stackBool);	
		while(!SEmptyP(&stack))	//pokym nebude zasobnik prazdny
		{
			pointer = STopPopP(&stack);	//vrchol zasobiku 
			SPushP(&stack, pointer);	
			left = STopPopB(&stackBool);
			if (!left)	//prichadza zprava, odstrani a spracuje
			{
				STopPopP(&stack);
				BTWorkOut(pointer);
			}
			else	//prichadza zlava ide do prava
			{
				SPushB(&stackBool,FALSE);
				Leftmost_Postorder(pointer->RPtr, &stack, &stackBool);
			}
		}
	}
}
...
**/
	if (*RootPtr != NULL)
	{
		...
		do
		{
			if(*RootPtr != NULL)	
			{
				if ((*RootPtr)->RPtr != NULL)	//praveho do zasobniku
				{
					SPushP(&stack, (*RootPtr)->RPtr);
				}
				pointer = *RootPtr;	
				(*RootPtr) = (*RootPtr)->LPtr;	//posun do lava
				free(pointer);	//zrusenie aktualneho uzla
			}
			else	//nie je koren
			{
				if (!SEmptyP(&stack))	//nie je prazdny zasobnik, zoberiem ho odtial
				{

...
*/
	if (q == NULL)	/*Pripad ze q == NULL*/
	{	
		queueError(QERR_INIT);
	}
	else
	{
		for (int i = 0; i < QUEUE_SIZE; i++)	/*Prejdenie vsetkych prvkov pola*/
		{
			q->arr[i] = '*';	/*Nastavenie hodnot v poli*/
		}
		q->f_index = 0;	/*Vynulovanie indexu zaciatku fronty*/
		q->b_index = 0;	/*Vynulovanie indexu prveho volneho miesta*/
	}
}
...
*/
	return(q->b_index == q->f_index);	/*Ak sa prvy prvok rovna poslednemu je fronta prazdna*/
}
...
*/
	return(q->f_index == nextIndex(q->b_index));	/*Ak sa dalsi rovna prvemu prvku fronta je plna*/
}
...
*/
	if (!(queueEmpty(q)))	/*Fronta nie je prazdna*/
	{
		*c = q->arr[q->f_index];	/*Vratenie znaku zo zaciatku fronty*/
	}
	else
	{
...
*/
	if (!(queueEmpty(q)))	/*Fronta nie je prazdna*/
	{	
		q->f_index = nextIndex(q->f_index);	/*Posunutie*/
	}
	else
	{
		queueError(QERR_REMOVE);	/*Prazdna fronta*/
	}
}
...
*/
	if (!(queueEmpty(q)))	/*Fronta nie je prazdna*/
	{
		queueFront(q, c);	/*Vratenie parametru c*/
		queueRemove(q);	/*Odstranenie znaku*/
	}
	else
	{
...
*/
	if (!queueFull(q))	/*Ak fronta nie je plna*/
	{
		q->arr[q->b_index] = c;	/*Vlozenie znaku c do fronty*/
		q->b_index = nextIndex(q->b_index);	/*Posunutie*/
	}
	else
	{
		queueError(QERR_UP);
